...
MD = 1  # I've only done it for 1 right now. We can average over 12 when we get one working?
...
parameters = [
    ...
    {'file_num': '4', 'legend_title': 'No Stacking', 'colour': 'blue'},
    {'file_num': '5', 'legend_title': 'Older Version of oxDNA\n(stacking not explicitly defined)', 'colour': 'black'}
...
for params in parameters:
    ...
    plt.plot(average_of_rep*0.85/100, force_array*48.6, 'o', color=params['colour'])
    plt.plot(average_of_rep*0.85/100, force_array*48.6, '--', linewidth=0.5, color=params['colour'], label=params['legend_title'])
